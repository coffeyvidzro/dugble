    ...
    <div className="space-y-8">
      <div className="rounded-2xl border bg-card/60 p-6">
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          <label htmlFor="email-volume" className="text-sm font-medium">
            Emails per month
          </label>
        ...
        {volume > 500000 && (
          <p className="mt-4 rounded-lg border border-pending/30 bg-pending/5 px-3 py-2 text-xs text-muted-foreground">
            For sustained volume above 500,000/month, talk to us directly. This
            estimate may not reflect the best available rate.
          </p>
        )}
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {costs.map(({ plan, cost }) => {
          const isCheapest = cheapest?.plan.name === plan.name && cost !== null;
          return (
            <div
              key={plan.name}
              className={cn(
                ...
                isCheapest && !plan.highlight && "border-signal/40",
              )}
            >
              {plan.highlight && (
