...
import { ArrowRight, Command, CornerDownLeft, SearchX, X } from "lucide-react";
...
export function CommandPalette() {
  ...
  const scope: CommandPaletteScope = pathname.startsWith("/dashboard")
    ? "dashboard"
    : "marketing";
  ...
  const commandItems = useMemo(() => getCommandItems(scope), [scope]);
  const groupOrder = useMemo(() => getGroupOrder(scope), [scope]);
  ...
  }, [query, commandItems, groupOrder]);

  const flatResults: CommandItem[] = useMemo(
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("dugble:open-command-palette", onOpenEvent);
  }, []);
  ...
  return (
    ...
    >
      <button
        ...
        onClick={() => setOpen(false)}
      />
      <div className="relative z-10 h-fit w-full max-w-xl overflow-hidden rounded-2xl border bg-card shadow-2xl shadow-black/50">
        <div className="flex items-center gap-3 border-b px-4">
          ...
          <input
            ...
            placeholder={
              scope === "dashboard"
                ? "Search pages, keys, logs…"
                : "Search pages, docs, updates…"
            }
        ...
        <div className="max-h-[60vh] overflow-y-auto p-2">
          {flatResults.length === 0 ? (
            ...
            results.map((section) => (
              <div key={section.group} className="py-2 first:pt-1">
    ...
    <div className="flex flex-col items-center gap-3 px-6 py-12 text-center">
      ...
      <div className="space-y-1">
        ...
        <p className="text-xs text-muted-foreground">
          Try a different term, or reach out if you can&apos;t find what you
          need.
        </p>
      </div>
      <Link
