...
Set `scheduled_at` to charge and queue an SMS now while delaying its delivery job until a future time.
...
Dugble changes the SMS state to `canceled`, removes its pending delivery event, and refunds the original SMS charge in the same database transaction.
