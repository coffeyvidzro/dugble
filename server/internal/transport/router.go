...
func NewRouter(cfg *config.Config, deps Dependencies) (*echo.Echo, error) {
	...
	tenantAccess := func(permission tenant.Permission) echo.MiddlewareFunc {
		return middlewares.TenantAccess(middlewares.TenantAccessConfig{
			Sessions: sessionRepository, Users: authRepository, Memberships: teamRepository,
			Tokens: teamTokenRepository, CSRF: csrfConfig, Required: permission,
		})
	}
	...
	teamtoken.RegisterRoutes(router, teamtoken.NewHandler(teamTokenService), authMiddleware, csrfMiddleware, tenantMiddleware)
	senderIDService := senderid.NewService(senderIDRepository)
	senderid.RegisterRoutes(router, senderid.NewHandler(senderIDService), authMiddleware, csrfMiddleware, tenantMiddleware)
	domainService := domain.NewService(domainRepository)
	domain.RegisterRoutes(router, domain.NewHandler(domainService), authMiddleware, csrfMiddleware, tenantMiddleware)
	...
	session.RegisterRoutes(router, session.NewHandler(sessionService), authMiddleware, csrfMiddleware)
