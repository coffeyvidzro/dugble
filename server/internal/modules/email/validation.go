...
func validateSend(req SendRequest, config ServiceConfig) (validatedSend, error) {
	...
	if len(idempotencyKey) > maxIdempotencyKeyBytes {
		return validatedSend{}, apperrors.NewBadRequest("Idempotency-Key must be at most 255 bytes")
	}
	...
	if idempotencyKey != "" {
		...
		hash, hashErr := validated.requestHash()
		if hashErr != nil {
			return validatedSend{}, apperrors.NewInternal("Unable to hash email request", hashErr)
		}
		validated.IdempotencyHash = &hash
	}
	return validated, nil
}

func (v validatedSend) requestHash() (string, error) {
