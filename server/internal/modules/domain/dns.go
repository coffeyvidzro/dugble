...
type NetDNSVerifier struct{ resolver *net.Resolver }

func NewNetDNSVerifier() *NetDNSVerifier { return &NetDNSVerifier{resolver: net.DefaultResolver} }
...
func (v *NetDNSVerifier) Verify(ctx context.Context, domain string, record VerificationRecord) bool {
	...
	case RecordTypeMX:
		...
		for _, value := range values {
			if strings.TrimSuffix(strings.ToLower(value.Host), ".") == expected && (record.Priority == nil || int(value.Pref) == *record.Priority) {
				return true
			}
		}
	}
