...
func RegisterRoutes(router *echo.Echo, handler *Handler, authMiddleware, csrfMiddleware echo.MiddlewareFunc, tenantMiddleware TenantMiddleware) {
	...
	router.POST("/auth/workload/oidc/token", handler.ExchangeOIDC)
	...
	group.GET("/:workload_id/oidc-federations", handler.ListOIDCFederations, tenantMiddleware(tenant.PermissionWorkloadsRead))
	group.POST("/:workload_id/oidc-federations", handler.CreateOIDCFederation, tenantMiddleware(tenant.PermissionWorkloadsWrite))
	group.DELETE("/:workload_id/oidc-federations/:federation_id", handler.DeleteOIDCFederation, tenantMiddleware(tenant.PermissionWorkloadsWrite))
