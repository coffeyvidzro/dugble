...
func (c *Client) Send(ctx context.Context, req sms.SendRequest) (sms.SendResult, error) {
	if c.apiKey == "" {
		return sms.SendResult{}, providerError("missing_api_key", "mNotify/BMS API key is not configured", false, false, false, nil)
	}
	endpoint, err := url.Parse(c.url)
	if err != nil {
		return sms.SendResult{}, providerError("invalid_url", "mNotify/BMS URL is invalid", false, false, false, err)
	}
	...
	if err != nil {
		return sms.SendResult{}, providerError("build_request", "Unable to build mNotify/BMS request", false, false, false, err)
	}
	httpReq.Header.Set("Accept", "application/json")
	httpRes, err := c.http.Do(httpReq)
	if err != nil {
		return sms.SendResult{}, providerError("request_failed", "mNotify/BMS request failed", true, true, false, err)
	}
	defer httpRes.Body.Close()
	body, err := io.ReadAll(httpRes.Body)
	if err != nil {
		return sms.SendResult{}, providerError("read_response", "Unable to read mNotify/BMS response", true, true, false, err)
	}
	if httpRes.StatusCode < 200 || httpRes.StatusCode >= 300 {
		return sms.SendResult{}, providerError(fmt.Sprintf("http_%d", httpRes.StatusCode), string(body), isTemporaryHTTPStatus(httpRes.StatusCode), isTemporaryHTTPStatus(httpRes.StatusCode), false, nil)
	}
