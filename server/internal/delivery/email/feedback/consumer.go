...
func normalizeConsumerConfig(config ConsumerConfig) ConsumerConfig {
	if config.Concurrency <= 0 { config.Concurrency = 5 }
	if config.AckWait <= 0 { config.AckWait = time.Minute }
	if config.HandlerTimeout <= 0 { config.HandlerTimeout = 30 * time.Second }
	if len(config.RetryPolicy.Delays) == 0 { config.RetryPolicy = DefaultRetryPolicy() }
	if config.MaxDeliver <= 0 { config.MaxDeliver = len(config.RetryPolicy.Delays) }
	return config
}
...
func (c *Consumer) Run(ctx context.Context) error {
	...
	if err != nil { return fmt.Errorf("provision email feedback consumer: %w", err) }
	...
	for workerIndex := range c.config.Concurrency {
		consumeContext, consumeErr := consumer.Consume(
			...
			natsjs.ConsumeErrHandler(func(_ natsjs.ConsumeContext, consumeErr error) {
				if consumeErr != nil { slog.Warn("email feedback consumer encountered a JetStream error", "worker", workerIndex, "error", consumeErr) }
			}),
		)
		if consumeErr != nil {
			for _, active := range contexts { active.Stop() }
		...
		go func(index int, active natsjs.ConsumeContext) {
			<-active.Closed()
			if ctx.Err() == nil { select { case errorsChannel <- fmt.Errorf("email feedback consumer worker %d stopped unexpectedly", index): default: } }
		}(workerIndex, consumeContext)
	}
	var runErr error
	select { case <-ctx.Done(): case runErr = <-errorsChannel: }
	for _, active := range contexts { active.Drain() }
	for _, active := range contexts { select { case <-active.Closed(): case <-time.After(c.config.HandlerTimeout): active.Stop() } }
	return runErr
}

func (c *Consumer) processMessage(parent context.Context, message natsjs.Msg) {
	metadata, err := message.Metadata()
	if err != nil { c.retry(message, 1, err); return }
	event, err := decodeProviderEventReference(message.Data(), message.Headers())
	if err != nil { c.deadLetter(parent, message, metadata, uuid.Nil, err); return }
	processed, err := c.processed.IsProcessed(parent, ConsumerName, event.EventID)
	if err != nil { c.retry(message, metadata.NumDelivered, err); return }
	if processed { c.ack(parent, message, event.EventID); return }
	...
	if err != nil {
		...
		if int(metadata.NumDelivered) >= c.config.MaxDeliver { c.deadLetter(parent, message, metadata, event.EventID, err); return }
		c.retry(message, metadata.NumDelivered, err); return
	}
	...
	}); err != nil { c.retry(message, metadata.NumDelivered, err); return }
...
func decodeProviderEventReference(data []byte, headers nats.Header) (ProviderEventReference, error) {
	...
	if err := json.Unmarshal(data, &event); err != nil { return ProviderEventReference{}, fmt.Errorf("decode email feedback event reference: %w", err) }
	if event.EventID == uuid.Nil || strings.TrimSpace(event.Provider) != ProviderSES { return ProviderEventReference{}, errors.New("invalid email feedback event reference") }
	headerEventID, err := uuid.Parse(strings.TrimSpace(headers.Get("Dugble-Event-Id")))
	if err != nil || headerEventID != event.EventID { return ProviderEventReference{}, errors.New("email feedback event ID does not match the outbox header") }
	if provider := strings.TrimSpace(headers.Get("Dugble-Provider")); provider != "" && provider != ProviderSES { return ProviderEventReference{}, errors.New("email feedback provider does not match the outbox header") }
	return event, nil
}

func (c *Consumer) retry(message natsjs.Msg, delivered uint64, cause error) {
	delay := c.config.RetryPolicy.Delay(delivered)
	if err := message.NakWithDelay(delay); err != nil { slog.Error("failed to negatively acknowledge email feedback event", "error", err, "cause", cause); return }
	slog.Warn("email feedback event will be retried", "delivery", delivered, "delay", delay, "error", cause)
}
...
func (c *Consumer) ack(parent context.Context, message natsjs.Msg, eventID uuid.UUID) {
	...
	if err := message.DoubleAck(ctx); err != nil { slog.Warn("failed to confirm email feedback acknowledgement", "event_id", eventID, "error", err) }
}
...
func (c *Consumer) deadLetter(ctx context.Context, message natsjs.Msg, metadata *natsjs.MsgMetadata, eventID uuid.UUID, cause error) {
	...
	for key, values := range message.Headers() { if len(values) > 0 { headers[key] = values[0] } }
	...
	if eventID == uuid.Nil { messageID = fmt.Sprintf("%s-%d-dlq", ConsumerName, metadata.Sequence.Stream) }
	if err := c.publisher.Publish(ctx, DLQSubject, message.Data(), headers, messageID); err != nil { c.retry(message, metadata.NumDelivered, fmt.Errorf("publish email feedback event to DLQ: %w", err)); return }
	if err := message.TermWithReason(truncateDeadLetterReason(cause)); err != nil { slog.Error("failed to terminate dead-lettered email feedback event", "event_id", eventID, "error", err); return }
	slog.Error("email feedback event moved to DLQ", "event_id", eventID, "delivery", metadata.NumDelivered, "error", cause)
}

func truncateDeadLetterReason(err error) string {
	if err == nil { return "unknown email feedback failure" }
	reason := strings.TrimSpace(err.Error())
	if reason == "" { return "unknown email feedback failure" }
	if len(reason) > 512 { return reason[:512] }
	return reason
}
