...
func (r *Repository) emitLifecycleWebhook(
	...
	webhookEvent, ok, err := emailLifecycleWebhookEvent(
		providerEventID,
		messageID,
		teamID,
		messageStatus,
...
func loadLifecycleRecipients(
	ctx context.Context,
	tx pgx.Tx,
	messageID uuid.UUID,
	recipients []string,
) ([]emailLifecycleRecipient, error) {
	...
	rows, err := tx.Query(ctx, `
		SELECT recipient_email, status,
			...
			COALESCE(last_status_code, ''),
			COALESCE(last_diagnostic_code, '')
...
func emailLifecycleWebhookEvent(
	...
	payload, err := json.Marshal(emailLifecyclePayload{
		Object:            "email",
		ID:                messageID.String(),
		Status:            strings.TrimSpace(messageStatus),
