...
Phase 2 pins OpenCV Zoo's YuNet detector and SFace embedder. Phase 3 pins Google's MediaPipe Face Landmarker task bundle. Phase 5 pins a MiniFAS ONNX classifier for supported two-dimensional presentation attacks. Each initializes through its reviewed runtime adapter. An `external` artifact is valid manifest metadata but cannot satisfy runtime readiness until a reviewed loader for that runtime is registered.
